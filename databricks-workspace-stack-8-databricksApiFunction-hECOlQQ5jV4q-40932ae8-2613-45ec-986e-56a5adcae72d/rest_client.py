...
def create_storage_configurations(account_id, storage_config_name, s3bucket_name, role_arn, encodedbase64, user_agent):
...
def create_workspaces(account_id, workspace_name, deployment_name, aws_region, credentials_id, storage_config_id, encodedbase64, network_id, customer_managed_key_id, pricing_tier, customer_name, authoritative_user_email, authoritative_user_full_name, user_agent):
...
def put_request(url, json_data, encodedbase64, user_agent, version):
    # sending put request and saving the response as response object
    resp = requests.put(url, json=json_data, headers={"Authorization": "Bearer %s" % encodedbase64, "User-Agent": "%s - %s" % (user_agent, version), "X-Databricks-Account-Console-API-Version": "2.0"})
    ...
    print('Successful PUT call!!')
    return data

# POST request function
def post_request(url, json_data, encodedbase64, user_agent, version):
    # sending post request and saving the response as response object
    resp = requests.post(url, json=json_data, headers={"Authorization": "Bearer %s" % encodedbase64, "User-Agent": "%s - %s" % (user_agent, version), "X-Databricks-Account-Console-API-Version": "2.0"})
...
def patch_request(url, json_data, encodedbase64, user_agent, version):
    # sending patch request and saving the response as response object
    resp = requests.patch(url, json=json_data, headers={"Authorization": "Basic %s" % encodedbase64, "User-Agent": "%s - %s" %(user_agent, version), "X-Databricks-Account-Console-API-Version": "2.0"})
    ...
    print('Successful PATCH call!!')
    return data

# GET request function
def get_request(url, encodedbase64, user_agent, version):
    # sending get request and saving the response as response object 
    resp = requests.get(url=url, headers={"Authorization": "Bearer %s" % encodedbase64, "User-Agent": "%s - %s" % (user_agent, version), "X-Databricks-Account-Console-API-Version": "2.0"})
